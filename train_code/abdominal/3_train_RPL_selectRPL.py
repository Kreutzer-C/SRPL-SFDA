# -*- coding: utf-8 -*-
"""
SFDA Step 3: Reliable Pseudo-Label Selection + Fine-tuning.
Loads source model, filters reliable pseudo-labels via entropy threshold,
trains with weighted CE+Dice on reliable regions only.
...
from utils.losses import WeightedDiceLoss, WeightedCrossEntropyLoss
...
parser.add_argument('--exp', type=str, default='SFDA_step3_RPL')
...
def train():
    ...
    for epoch_num in range(1, max_epoch + 1):
        for batch in tqdm(trainloader, desc=f'Epoch {epoch_num}/{max_epoch}', ncols=70):
            ...
            unreliable_mask = (um > threshold).float()
