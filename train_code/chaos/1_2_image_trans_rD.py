"""
Stage 1-2: Apply random-dark gamma correction (γ=0.6) and run source model.
Reads from:  {data_dir}/{domain}/slices/
Writes to:   {data_dir}/{domain}/slices_pred_rD/   (pred_prob [C,H,W])
"""
import argparse
import json
import os
...
def gamma_correction(img_2d: np.ndarray, gamma: float) -> np.ndarray:
    """Min-max normalise then apply power-law gamma, return float32 [0,1]."""
