#!/bin/bash
# SRPL-SFDA: ABDOMINAL (BTCV → CHAOST2) — Faithful Reproduction
#
# This is the original entry point. For bidirectional support, see:
#   run_btcv2chaost2.sh  (CT→MR, same as this script)
#   run_chaost22btcv.sh  (MR→CT, reverse direction)
#
# The original SRPL-SFDA SAM bbox refinement (Phase 2) is designed for
# binary segmentation and cannot be directly extended to multi-class.
# This pipeline reproduces all applicable components:
#   Phase 0: Source model training (BTCV)
#   Phase 1: Tri-branch intensity enhancement + ensemble pseudo-labels
#   Phase 2: RPL selection + fine-tune (Step 3 ablation)
